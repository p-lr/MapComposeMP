...
class ExprSerializer<T : Any>(
                        ...
                        listOf(
                            JsonPrimitive(
                                when (value.interpolation) {
                                    ...
                                    is InterpolationType.CubicBezier -> "cubic-bezier"
                                }
                            )
                        )
                    ),
    ...
    private fun serializeExpr(expr: Expr<@Contextual Any?>, jsonEncoder: JsonEncoder): JsonElement {
        return when (expr) {
            is Expr.Get -> JsonArray(
                listOf(
                    JsonPrimitive("get"),
    ...
    private fun serializeIndexOf(value: Expr.IndexOf<T>, encoder: JsonEncoder) {
        encoder.encodeJsonElement(
            JsonArray(
                listOf(
    ...
    override fun serialize(encoder: Encoder, value: Expr<T>) {
        ...
        when (value) {
            ...
            is Expr.LessThan -> serializeLessThan(value, jsonEncoder)
            is Expr.LessThanOrEqual -> serializeLessThanOrEqual(value, jsonEncoder)
            is Expr.GreaterThan -> serializeGreaterThan(value, jsonEncoder)
            is Expr.GreaterThanOrEqual -> serializeGreaterThanOrEqual(value, jsonEncoder)
    ...
    private fun deserializeZoomStops(
        ...
        val stops: List<Pair<Double, Expr<@Contextual Any?>>> = element[1].jsonArray.map { stop ->
            ...
            val value: Expr<@Contextual Any?> = deserializeExpr(element = stop[1], jsonDecoder = jsonDecoder, isNested = true)
            Pair(
                first = zoom,
                second = value,
            )
    ...
    private fun deserializeCase(element: JsonArray, jsonDecoder: JsonDecoder, isNested: Boolean): Expr.Case<T> {
        ...
        val conditions = mutableListOf<Pair<Expr<@Contextual Any?>, Expr<T>>>()
        for (i in 1 until element.size - 1 step 2) {
            ...
            val resultEl = element[i + 1]
            val result = deserializeExpr(element = resultEl, jsonDecoder = jsonDecoder, isNested = resultEl !is JsonPrimitive) as Expr<T>
            conditions.add(condition to result)
        }
        val defaultEl = element.last()
        val default = deserializeExpr(element = defaultEl, jsonDecoder = jsonDecoder, isNested = defaultEl !is JsonPrimitive) as Expr<T>
    ...
    private fun deserializeCoalesce(element: JsonArray, jsonDecoder: JsonDecoder, isNested: Boolean): Expr.Coalesce<T> {
        ...
        val values = element.drop(1).map { valueElement ->
            deserializeExpr(element = valueElement, jsonDecoder = jsonDecoder, isNested = valueElement !is JsonPrimitive) as Expr<T>
        }
        ...
        val interpolationType = when (val interpolationElement = element[1]) {
            ...
            is JsonArray -> when (interpolationElement[0].jsonPrimitive.content) {
                ...
                "cubic-bezier" -> InterpolationType.CubicBezier(
                    x1 = interpolationElement[1].jsonPrimitive.double,
                    y1 = interpolationElement[2].jsonPrimitive.double,
                    x2 = interpolationElement[3].jsonPrimitive.double,
                    y2 = interpolationElement[4].jsonPrimitive.double
                )
                else -> throw SerializationException("Invalid interpolation type")
            }
    ...
    private fun deserializeAt(element: JsonArray, jsonDecoder: JsonDecoder): Expr.At<T> {
        ...
        val array = deserializeExpr(element[2], jsonDecoder, isNested = true)
    ...
    private fun deserializeLength(element: JsonArray, jsonDecoder: JsonDecoder): Expr.Length<T> {
        ...
        val array = deserializeExpr(element = element[1], jsonDecoder = jsonDecoder, isNested = true)
    ...
    private fun deserializeSlice(element: JsonArray, jsonDecoder: JsonDecoder): Expr.Slice<T> {
        ...
        val array = deserializeExpr(element[1], jsonDecoder, isNested = true)
    ...
    private fun deserializeLessThan(input: JsonArray, jsonDecoder: JsonDecoder, isNested: Boolean): Expr<@Contextual Any?> {
        if (input.size != 3) throw SerializationException("Invalid 'less than' expression format")
        return Expr.LessThan(
            deserializeExpr(element = input[1], jsonDecoder = jsonDecoder, isNested = true),
            deserializeExpr(element = input[2], jsonDecoder = jsonDecoder, isNested = true)
        ) as Expr<Boolean>
    }

    private fun deserializeLessThanOrEqual(input: JsonArray, jsonDecoder: JsonDecoder, isNested: Boolean): Expr<@Contextual Any?> {
    ...
    private fun deserializeGreaterThan(input: JsonArray, jsonDecoder: JsonDecoder, isNested: Boolean): Expr<@Contextual Any?> {
    ...
    private fun deserializeGreaterThanOrEqual(input: JsonArray, jsonDecoder: JsonDecoder, isNested: Boolean): Expr<@Contextual Any?> {
    ...
    private fun deserializeConcat(element: JsonArray, jsonDecoder: JsonDecoder, isNested: Boolean): Expr.Concat<T> {
        if (element.size < 3) throw SerializationException("Invalid concat expression: needs at least 2 arguments")
        val args = (1 until element.size).map { i ->
            deserializeExpr(element = element[i], jsonDecoder = jsonDecoder, isNested = true)
        }
        @Suppress("UNCHECKED_CAST")
        return args.drop(1).fold(args.first()) { acc, expr -> Expr.Concat<T>(acc, expr) } as Expr.Concat<T>
    }
    ...
    private fun deserializeIndexOf(element: JsonArray, jsonDecoder: JsonDecoder, isNested: Boolean): Expr.IndexOf<T> {
        ...
        val array = deserializeExpr(element = element[1], jsonDecoder = jsonDecoder, isNested = true)
        val value = deserializeExpr(element = element[2], jsonDecoder = jsonDecoder, isNested = true)
    ...
    // ["format", value1, opts1, value2, opts2, ...] — values at odd indices, option objects at even indices.
    // Extracts each value, concatenates them, or returns null if empty.
    private fun deserializeFormat(element: JsonArray, jsonDecoder: JsonDecoder, isNested: Boolean): Expr<@Contextual Any?> {
        val parts = (1 until element.size step 2)
            .filter { element[it] !is JsonObject }
            .map { deserializeExpr(element = element[it], jsonDecoder = jsonDecoder, isNested = true) }
        return parts.firstOrNull() ?: Expr.Constant(null)
    }
    ...
    private fun deserializeStep(element: JsonArray, jsonDecoder: JsonDecoder, isNested: Boolean): Expr.Step<T> {
        ...
        while (i < element.size - 1) {
            val stop = element[i++].jsonPrimitive.double
            val valueEl = element[i++]
            val value = deserializeExpr(element = valueEl, jsonDecoder = jsonDecoder, isNested = valueEl !is JsonPrimitive) as Expr<T>
    ...
    fun deserializeExpr(element: JsonElement, jsonDecoder: JsonDecoder, isNested: Boolean): Expr<@Contextual Any?> {
        return when (element) {
            ...
            is JsonArray -> {
                ...
                when (operator) {
                    ...
                    "format" -> deserializeFormat(element, jsonDecoder, isNested)
                    else -> Expr.Constant(null)
                }
            }
        }
    }
